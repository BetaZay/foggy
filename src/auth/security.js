...
const PRE_AUTH_CSRF_SECRET = crypto.randomBytes(32);
...
function preAuthSignature(purpose, payload) {
  return crypto.createHmac('sha256', PRE_AUTH_CSRF_SECRET)
...
function issueSignedCsrf(purpose, now) {
  const payload = `${now}.${crypto.randomBytes(24).toString('base64url')}`;
  return `${payload}.${preAuthSignature(purpose, payload)}`;
}

function verifySignedCsrf(purpose, token, now) {
  ...
  return equalToken(signature, preAuthSignature(purpose, `${issuedValue}.${nonce}`));
}

export function issuePreAuthCsrf(now = Date.now()) {
  return issueSignedCsrf('login', now);
}

export function verifyPreAuthCsrf(token, now = Date.now()) {
  return verifySignedCsrf('login', token, now);
}

export function issueServerConfigCsrf(now = Date.now()) {
  return issueSignedCsrf('server-config', now);
}

export function verifyServerConfigCsrf(token, now = Date.now()) {
  return verifySignedCsrf('server-config', token, now);
}
...
export function requireLoginCsrf(request, response, next) {
  if (!verifyPreAuthCsrf(request.body?._csrf)) {
    securityLog('login_csrf_rejected', request, {
